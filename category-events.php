...
//     $query="SELECT * FROM events WHERE CONCAT(date_of_event_organize, ' ', events_time_duration) > '$now' AND title like '%$search%' ORDER BY date_of_event_organize ASC LIMIT 6";
...
if((isset($_GET['id']))){
    ...
    $now = date('Y-m-d H:i:s');
    $query="SELECT * FROM events WHERE CONCAT(date_of_event_organize, ' ', events_time_duration) > '$now' AND category_events_id = $search AND status = 1 ORDER BY date_of_event_organize ASC LIMIT 6";
    $posts=mysqli_query($conn,$query);
